...
// Total data
$total_data = mysqli_query($koneksi, "SELECT COUNT(*) as total FROM t_user WHERE role_user='staff'");
...
if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id'])) {
    ...
    if (!empty($errors)) {
        ...
        $delete = mysqli_query($koneksi, "DELETE FROM t_user WHERE id_user = '$id' AND role_user = 'staff'");
        if ($delete) {
            $_SESSION['success_message'] = "Data staff berhasil dihapus!";
        } else {
            $_SESSION['error_message'] = "Gagal menghapus data: " . mysqli_error($koneksi);
        }
    }
    header("Location: data_staff.php" . ($page > 1 ? "?page=$page" : ""));
    exit();
}

// Proses Edit Data (Hanya edit, tidak ada proses add untuk koordinator)
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'edit') {
    ...
    if ($result) {
        $_SESSION['success_message'] = "Data staff berhasil diupdate!";
    } else {
...
if (isset($_GET['action']) && $_GET['action'] == 'edit' && isset($_GET['id'])) {
    $id = mysqli_real_escape_string($koneksi, $_GET['id']);
    $query_edit = mysqli_query($koneksi, "SELECT * FROM t_user WHERE id_user = '$id' AND role_user = 'staff'");
...
<div class="main-content">
    <section class="section">
        <div class="section-header">
            <h1><i class="fas fa-chalkboard-teacher mr-2"></i>Data Staff</h1>
        ...
        <div class="section-body">
            ...
            <!-- STATS CARDS -->
            <?php
            $total_staff = mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM t_user WHERE role_user='staff'"));
            $total_matkul_diampu = mysqli_fetch_assoc(mysqli_query($koneksi, "
                SELECT COUNT(DISTINCT j.id_jdwl) as total, SUM(j.jml_mhs) as total_mhs
                FROM t_jadwal j 
                INNER JOIN t_user u ON j.id_user = u.id_user 
                WHERE u.role_user = 'staff'
            "));
            ?>
            <div class="up-stat-row">
                <div class="up-stat-card">
                    <div class="up-stat-value"><?= $total_staff ?></div>
                    <div class="up-stat-label">Total Staff</div>
                </div>
                <div class="up-stat-card">
                ...
                <div class="up-stat-card">
                    <div class="up-stat-value"><?= $total_matkul_diampu['total_mhs'] ?? 0 ?></div>
            ...
            <div class="up-main-card">
                <div class="up-main-card-header">
                    ...
                    <h5>Daftar Staff / Dosen</h5>
                    <div class="ml-auto d-flex align-items-center gap-2">
                        <div class="up-search-box" style="width: 250px;">
                            <i class="fas fa-search"></i>
                            <input type="text" class="up-search-input" id="searchInput" placeholder="Cari staff..." onkeyup="filterTable()">
                        </div>
                    ...
                    <div class="up-table-responsive">
                        <table class="up-table up-table-hover" id="dataTable">
                            ...
                            <tbody>
                                <?php
                                $no = $offset + 1;
                                $staff = mysqli_query($koneksi, "SELECT * FROM t_user WHERE role_user='staff' ORDER BY nama_user ASC LIMIT $offset, $limit");
                                while ($stf = mysqli_fetch_assoc($staff)) {
                                ?>
                                <tr>
                                    <td><span class="up-badge up-badge-default"><?= $no++ ?></span></td>
                                    <td><strong><?= htmlspecialchars($stf['npp_user']) ?></strong></td>
                                    <td><?= htmlspecialchars($stf['nik_user']) ?: '-' ?></td>
                                    <td><?= htmlspecialchars($stf['npwp_user']) ?: '-' ?></td>
                                    <td><?= htmlspecialchars($stf['norek_user']) ?: '-' ?></td>
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <div class="up-avatar-sm bg-success text-white mr-2">
                                                <?= strtoupper(substr($stf['nama_user'], 0, 1)) ?>
                                            </div>
                                            <?= htmlspecialchars($stf['nama_user']) ?>
                                        </div>
                                    </td>
                                    <td><?= htmlspecialchars($stf['nohp_user']) ?: '-' ?></td>
                                    <td>
                                        <span class="up-badge up-badge-success">
                                            Rp <?= number_format($stf['honor_persks'], 0, ',', '.') ?>
                                        </span>
                                    </td>
                                    <td>
                                        <div class="btn-group">
                                            <a href="#" onclick="viewData(<?= $stf['id_user'] ?>)" class="up-btn-icon up-btn-icon-info" title="Detail">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            <a href="#" onclick="editData(<?= $stf['id_user'] ?>)" class="up-btn-icon up-btn-icon-warning" title="Edit">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <a href="?action=delete&id=<?= $stf['id_user'] ?>&page=<?= $page ?>" class="up-btn-icon up-btn-icon-danger" title="Hapus" onclick="return confirm('Yakin ingin menghapus staff <?= htmlspecialchars($stf['nama_user']) ?>?')">
...
<!-- Modal Form Edit Staff (Hanya untuk Edit) -->
            ...
            <form action="" method="POST" id="staffForm">
                <div class="modal-body">
                    ...
                    <div class="up-form-grid" style="grid-template-columns: repeat(2, 1fr);">
                        ...
                        <div class="up-form-group">
                            <label class="up-form-label">Password</label>
                            <input type="password" class="up-input" name="pw_user" id="pw_user">
                            <small class="up-form-hint">Kosongkan jika tidak ingin mengubah password</small>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="up-btn up-btn-secondary" data-dismiss="modal">Batal</button>
                    <button type="submit" class="up-btn up-btn-primary">Simpan Perubahan</button>
                </div>
            </form>
        </div>
    </div>
...
// Fungsi openModal hanya untuk edit (tidak ada add)
...
function editData(id) {
    fetch('ajax/get_staff.php?id=' + id)
        ...
        .then(data => {
            if (data.success) {
                ...
                document.getElementById('pw_user').value = ''; // Kosongkan password
...
function viewData(id) {
    fetch('ajax/view_staff.php?id=' + id)
